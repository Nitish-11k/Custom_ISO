#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║          KIOSK LIVE ISO + PXE BUILDER  v4.0                              ║
# ║          Pure Debian Sid · Tauri v2 · BIOS/UEFI Hybrid                   ║
# ╚══════════════════════════════════════════════════════════════════════════╝
set -euo pipefail
IFS=$'\n\t'
...
# Debian (Default changed to 'sid' to natively support GLIBC 2.42)
DEBIAN_RELEASE="${DEBIAN_RELEASE:-sid}"
ARCH="${ARCH:-amd64}"
...
# ── APT SOURCES ──────────────────────────────────────────────────
if [ "RELEASE_PLACEHOLDER" = "sid" ]; then
...
# ── BACKPORTS UPGRADE — GLIBC 2.38 FIX (Skip if Sid) ─────────────
if [ "RELEASE_PLACEHOLDER" != "sid" ]; then
...
apt-get -qq install -y -t RELEASE_PLACEHOLDER-backports \
    ...
    libcairo2 \
    libgdk-pixbuf-2.0-0 \
    2>&1 || echo "  [WARN] Backports upgrade failed"
fi
...
# Mask plymouth auto-quit so it spins until AppImage is ready
...
# Dismiss the Plymouth Splash AFTER extraction is complete to show the app
slog "Stopping Plymouth spinner seamlessly..."
    ...
    if [ -n "$NEEDED" ]; then
        if [ "$(printf '%s\n%s' "$SYS_GLIBC_VER" "$NEEDED" | sort -V | tail -1)" != "$SYS_GLIBC_VER" ]; then
            if grep -qF "$LIB_NAME" /tmp/ldconfig_cache.txt 2>/dev/null; then
                # rm -f "$so_file"  <-- DISABLED FOR GLIBC 2.42 PRESERVATION
                slog "Skipped stripping: $LIB_NAME (to preserve GLIBC 2.42)"
                STRIPPED=$((STRIPPED + 1))
            else
            ...
            for lib_path in $NEW_GLIBC_LIBS; do
                if [ -f "$lib_path" ]; then
                    ...
                    if ldconfig -p 2>/dev/null | grep -qF "$LIB_BASE"; then
                        # rm -f "$lib_path" && slog "Hot-stripped: $lib_path (system has $LIB_BASE)" <-- DISABLED
                        slog "Hot-strip disabled for: $lib_path"
                    fi
                fi
            done
        fi
...
find /usr/bin /usr/sbin /usr/lib -type f -executable \
    ...
    -not -name '*dri*' -not -name '*mesa*' -not -name '*egl*' -not -name '*glx*' \
...
CHROOT_EOF
    ...
    cp -r "$WORKDIR/plymouth_theme/kiosk-spinner/"* "$ROOTFS_DIR/usr/share/plymouth/themes/kiosk-spinner/" 2>/dev/null || true
...
Usage: sudo $0 [OPTIONS]
  --release RELEASE   Debian release  (default: sid)
